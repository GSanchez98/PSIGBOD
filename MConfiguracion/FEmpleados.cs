        ...
        private void AgrEdit(int x)
        {
            if (x == 1) // GIMENA: Agregar
            {
                ...
                try
                {
                    ...
                    comando.Parameters.AddWithValue("@fecha_nacimiento_Empleado", txtFechaNac.Value.ToShortDateString().ToString());
                    comando.Parameters.AddWithValue("@fecha_ingreso_Empleado", txtFechaIng.Value.ToShortDateString().ToString());
